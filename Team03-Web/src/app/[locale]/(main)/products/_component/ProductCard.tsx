    ...
    return (
        ...
        >
            {product.discountPercentage > 0 && (
                <div className="absolute top-3 left-3 bg-gradient-to-r from-red-500 to-pink-500 text-white text-xs font-semibold px-3 py-1 rounded-full shadow-lg hover:scale-105 transition-transform duration-200 uppercase tracking-wider z-10">
                    {t('discount', { percent: product.discountPercentage.toLocaleString() })}
                </div>
            )}

            <div className="flex flex-col">
                <div className="relative w-full pb-full mb-4">
                    <img src={product.image} alt={product.productName} className="object-cover w-full h-full rounded-md" />
                </div>
                <h3 className="font-semibold text-sm md:text-base lg:text-lg mb-2 line-clamp-2">{product.productName}</h3>
            </div>
            ...
            <div className="flex flex-col">
                <div className="flex flex-col items-center justify-start gap-2 mb-1 sm:flex-row">
                    ...
                    {/* Only show oldPrice if newPrice is different */}
                    {product.newPrice !== product.oldPrice && (
                        <p className="text-gray-400 text-sm line-through">{formatMoney(product.oldPrice)}</p>
                    )}
                </div>
